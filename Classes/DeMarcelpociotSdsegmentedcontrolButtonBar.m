...
@implementation DeMarcelpociotSdsegmentedcontrolButtonBar
...
-(SDSegmentedControl *)segmentedControl
{
	if (segmentedControl==nil)
	{
		...
		segmentedControl = [[SDSegmentedControl alloc] init];
        segmentedControl.frame = ourBoundsRect;
...
-(void)frameSizeChanged:(CGRect)frame_ bounds:(CGRect)bounds_
{
    ...
    SDSegmentedControl* ourControl = [self segmentedControl];
...
{
	int newIndex = [(SDSegmentedControl *)sender selectedSegmentIndex];
	...
	if ([(SDSegmentedControl *)sender isMomentary])
	{
		selectedIndex = -1;
